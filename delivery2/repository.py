...
def unpack_signed_payload(signed_payload: dict, public_key: str) -> dict:
    print("signed_payload", signed_payload, flush=True)
...
def create_session():
    ...
    challenge = decrypt(encrypted_challenge, PRIVATEKEY)
    conn = sqlite3.connect(DATABASE)
    conn.row_factory = sqlite3.Row
    cur = conn.cursor()
    try:
...
def get_file():
    ...
    try:
        ...
        return jsonify(signed_payload({"error": "Invalid file_handle format. Must be Base64."})), 400
    ...
    try:
        ...
        findIfManager = query_db(
            ...
            * FROM Role WHERE Role.id = (
                SELECT role_id FROM Subject_Role WHERE subject_id = ? AND assumed = TRUE
            )
            """,
            (subject_id,),
            one=True
        )
        if findIfManager["name"] == "manager":
            return jsonify(signed_payload({"error": "Manager cannot be suspended"})), 403
        ...
        encrypted_response = encrypt(json.dumps({"message": "Subject suspended successfully"}), keys[0])
        return jsonify(signed_payload(encrypted_response)), 200
...
def activate_subject():
    ...
    try: 
        ...
        subject_roles = query_db("SELECT Role.id FROM Role JOIN Subject_Role ON Role.id = Subject_Role.role_id WHERE Subject_Role.subject_id = ? AND Subject_Org.status_ = 'active' AND Subject_Role.assumed = TRUE", (subject_id,))
        permissions = []
        ...
        if "SUBJECT_UP" not in permissions["org_permissions"]:
...
def list_docs():
    try:
        ...
        print("date_operator", date_operator)
        print("nonce", nonce)
        ...
        # se nao houver documentos na organizacao
        print("docs", docs)
        if not docs:
...
def delete_document():
    ...
    try:
        ...
        try:
            ...
            cur.execute("DELETE FROM Doc_Perms WHERE d_handle = ?", (doc["d_handle"],))
...
def suspend_role():
    ...
    try:
        ...
        session_pub_key = query_db("SELECT keys FROM Session WHERE subject_id = ? AND organization_id = ?", (subject_id, organization_id), one=True)
        session_pub_key = session_pub_key["keys"].replace("\\n", "\n")
        encrypted_response = encrypt("Role suspended successfully to organization", session_pub_key)
        return jsonify(signed_payload(encrypted_response)), 200
...
def reactivate_role():
    ...
    try:
        ...
        session_pub_key = query_db("SELECT keys FROM Session WHERE subject_id = ? AND organization_id = ?", (subject_id, organization_id), one=True)
        session_pub_key = session_pub_key["keys"].replace("\\n", "\n")
        encrypted_response = encrypt("Role reactivated successfully!", session_pub_key)
        return jsonify(signed_payload(encrypted_response)), 200
...
def rep_remove_permission_from_user():
    ...
    try:
        ...
        findIfSubjectExists = query_db("SELECT * FROM Subject WHERE username = ?", (username,), one=True)
        if role_name == "manager":
            numManager = query_db("SELECT COUNT(*) FROM Role WHERE organization_id = ? AND name = 'manager'", (organization_id,), one=True)
            print(numManager)
            if numManager["COUNT(*)"] == 1:
...
def modify_doc_acl():
    ...
    try:
        ...
        session_pub_key = query_db("SELECT keys FROM Session WHERE subject_id = ? AND organization_id = ?", (subject_id, organization_id), one=True)
        session_pub_key = session_pub_key["keys"].replace("\\n", "\n")
        encrypted_response = encrypt("Document ACL modified successfully", session_pub_key)
        return jsonify(signed_payload(encrypted_response)), 200
